...
blendColor (Color r1 g1 b1) (Color r2 g2 b2) =
  Color (average r1 r2) (average g1 g2) (average b1 b2) 
  where average x1 x2 = div (x1 + x2) 2

combine :: (Color -> Color -> Color) -> Picture -> Picture -> Picture
combine f (Picture p1) (Picture p2) = Picture go
  where go coord = f (p1 coord) (p2 coord)
...
rectangle x0 y0 w h = Shape s
  where s (Coord x y) =
          between x x0 (x0 + w - 1) && between y y0 (y0 + h - 1)
        between e1 e2 e3 = e1 >= e2 && e1 <= e3
...
paintSolid color shape base = Picture p
  where p coord = if sFunction shape $ coord
                    then color
                    else pFunction base $ coord

pFunction (Picture f) = f
sFunction (Shape s)   = s
...
paint pat shape base = Picture p
  where p coord = if sFunction shape $ coord
                    then pFunction pat $ coord
                    else pFunction base $ coord
...
instance Transform Fill where
  apply (Fill c) (Picture _) = Picture p
    where p coord = c
...
instance Transform Flip where
  apply FlipX (Picture p) = Picture go
    where go (Coord x y) = p (Coord (-x) y)
  apply FlipY (Picture p) = Picture go
    where go (Coord x y) = p (Coord x (-y))
  apply FlipXY (Picture p) = Picture go
    where go (Coord x y) = p (Coord y x)
...
instance Transform Blur where
  apply Blur (Picture p) = Picture go 
     where go (Coord x y) = blendNColors 
                              [(p   (Coord  x     y)),
                                (p (Coord (x+1)  y)),
                                (p (Coord  x    (y+1))),
                                (p (Coord  x    (y-1))),
                                (p (Coord (x-1)  y))]
           blendNColors cs = divColor (foldr sumColors (Color 0 0 0) cs) (length cs)
           sumColors (Color r1 g1 b1) (Color r2 g2 b2) =
             Color (r1 + r2) (g1 + g2) (b1 + b2)
           divColor (Color r g b) n = Color (div r n) (div g n) (div b n)
...
--  apply (BlurMany t) p = foldr apply p (take t $ repeat Blur)
    apply (BlurMany t) p = if t == 0
      then p
      else apply (BlurMany (t - 1)) (apply Blur p)
