...
buyOneMore (MkShoppingEntry e p c) = (MkShoppingEntry e p (c + 1))
...
tick :: UpDown -> UpDown
tick (MkUp c) = MkUp (c + 1)
tick (MkDown c) = MkDown (c - 1)

-- toggle changes an increasing counter into a decreasing counter and
-- vice versa
toggle :: UpDown -> UpDown
toggle (MkUp c) = MkDown c
toggle (MkDown c) = MkUp c
...
fromBin' :: Bin -> Int -> Int
fromBin' End _ = 0
fromBin' (O b) e = fromBin' b (e + 1)
fromBin' (I b) e = 2 ^ e + (fromBin' b (e + 1))

toBin :: Int -> Bin
toBin n = toBin' n (requiredBits n 1) End

toBin' :: Int -> Int -> Bin -> Bin
toBin' n 0    bin = bin
toBin' n bits bin = if 2 ^ (bits - 1) > n
  then toBin' n (bits - 1) (O bin)
  else toBin' (n - 2 ^ (bits - 1)) (bits - 1) (I bin)

requiredBits :: Int -> Int -> Int
requiredBits n b = if (2 ^ b - 1) >= n
  then b 
  else requiredBits n (b + 1) 
