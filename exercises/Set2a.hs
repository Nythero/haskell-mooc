...
isPalindrome str 
  | 2 > length str = True
  | otherwise = take 1 str == (drop droppedItems str)
    && isPalindrome (drop 1 (take droppedItems str))
    where droppedItems = (length str - 1)
...
greet first last = "Hello, " ++ first ++ case last of
  Just l -> " " ++ l
  Nothing -> ""
  ++ "!"
...
safeIndex xs i = if i >= (length xs) || 0 > i 
  then Nothing
  else Just (head (drop i xs))
