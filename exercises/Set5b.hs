...
treeMax Empty = 0
treeMax (Node v l r) = max (max v (treeMax l)) (treeMax r)
...
walk (s:ss) (Node v l r) = case s of StepL -> walk ss l
                                     StepR -> walk ss r
...
set (s:ss) val (Node v l r) = case s of StepL -> Node v (set ss val l) r
                                        StepR -> Node v l (set ss val r)
...
search a tree = if isInTree a tree 
  then Just (search' a tree)
  else Nothing

search' :: Eq a => a -> Tree a -> [Step]
search' a (Node v l r) = if a == v
  then []
  else if isInTree a l
    then StepL : search' a l
    else StepR : search' a r

isInTree :: Eq a => a -> Tree a -> Bool
isInTree _ Empty = False
isInTree v (Node v2 l r) = v == v2 || (isInTree v l) || (isInTree v r)
