...
firstHalf s = take (length s `div` 2 + length s `mod` 2) s
...
powers :: Int -> Int -> [Int]
powers k max = powers' k 0 max 

powers' :: Int -> Int -> Int -> [Int]
powers' k e max = if k^e > max
  then []
  else k^e : powers' k (e+1) max
...
joinToLength n ss = [s1 ++ s2 | s1 <- ss, s2 <- ss, (==n) . length $ s1 ++ s2]
...
sumRights is = sum $ map (either (const 0) id) is
...
interpreter' x y [] = []
interpreter' x y (c:cs) = case c of "up" -> interpreter' x (y + 1) cs
