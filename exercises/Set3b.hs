...
sums' i m
    | i == m = sumatory i : []
    | otherwise = sumatory i : sums' (i + 1) m 

sumatory :: Int -> Int 
sumatory 1 = 1
sumatory m = m + sumatory (m - 1)
...
sorted [] = True
sorted (x:[]) = True
sorted (x:x2:xs) = x <= x2 && sorted (x2:xs)
...
sumsOf' :: Int -> [Int] -> [Int]
sumsOf' _ [] = []
sumsOf' y (x:xs) = sum : sumsOf' sum xs
  where sum = y + x 
...
mymaximum bigger initial (x:xs) = mymaximum bigger big xs
    where 
        big | bigger initial x = initial
            | otherwise = x
